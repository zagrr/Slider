...
class Slider extends Component {
    ...
    nextOne() {
        ...
        (this.state.active < this.state.max - 1) ? this.setState({ active: this.state.active + 1 }) :
            this.setState({ active: 0 });
    }

    previousOne() {

        (this.state.active > 0) ? this.setState({ active: this.state.active - 1 }) :
